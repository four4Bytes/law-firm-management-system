...
- Centralized Auth Guards: Invoke unified, centralized protection functions at the very top of the execution flow: `requireAuth()` (verified session), `requirePermission(...permissions)` (context-free RBAC cells), or `requirePermissionOrNull(...permissions)` for write actions that return `ActionStatusResponse` instead of throwing. Record-scoped cells use `assertRecordPermission(session, permission, accessContext)` which throws "Forbidden" when denied. Do not write inline, ad-hoc `auth()` verification logic inside individual actions.
