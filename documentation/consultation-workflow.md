...
- **Create** (`ConsultationCreatePayloadSchema`): status is `Scheduled | Completed` only (`Completed` covers backfill of already-held meetings). There is no way to create an `Accepted`/`Rejected`/`Cancelled` consultation.
