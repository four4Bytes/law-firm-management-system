...
- Rule of thumb for RBAC action: do **not** hide row/action buttons (Edit, Delete, etc.) behind client-side RBAC checks. Always render the action and let the Server Action return the success/error message (`ActionStatusResponse`) when the user clicks it; surface that message on the client via the toast `queue` (`queue.add(...)`). This keeps the access-context decision server-side (the client doesn't know the record's access context) and lets the user see _why_ an action failed instead of the button silently missing. Client-side `can()` gating is otherwise reserved for coarse, context-free UI chrome (whole tabs, add buttons), never per-record row actions where the access context isn't known on the client. There are some cases where hiding the UI does make sense (better UX) and contradicts the rule above, if that happens let me know.
- **Read permission gating** (replaces the old "always hide View" exception): distinguish by the surface. If the read is a **button/row action** (e.g. a "View"/"Open" button), render it and let the Server Action surface the denial via the toast `queue` — do not hide it (same rule as other actions). If the read is **navigation** (a link/route to a page the user cannot open), hide the link or guard the navigation instead, since pointing a user at inaccessible content is worse than hiding it.
- Scan `src/lib/` for existing utilities before creating new types/functions to avoid duplication.
