...
RBAC is enforced from the declarative matrix in `src/lib/rbac.ts`, which mirrors [RBAC.md](./RBAC.md) cell-for-cell. Server Actions guard context-free writes using `requirePermission(...)`, while record-scoped writes use `requireAuth()` followed by `can(role, permission, access)` after loading the record's access context (`getCaseAccessContext`, `getConsultationAccessContext`, `getTaskAccessContext`, etc.).
