...
- Rule of thumb for RBAC action: do **not** hide row/action buttons (Edit, Delete, etc.) behind client-side RBAC checks. Always render the action and let the Server Action return the success/error message (`ActionStatusResponse`) when the user clicks it; surface that message on the client via the toast `queue` (`queue.add(...)`). This keeps the access-context decision server-side (the client doesn't know the record's access context) and lets the user see _why_ an action failed instead of the button silently missing. The single exception is **View/Read**: always hide the "View"/"Open" row action when the user lacks READ permission on the record — pointing a user at content they can't open is worse than hiding it. Client-side `can()` gating is otherwise reserved for coarse, context-free UI chrome (whole tabs, add buttons), never per-record row actions where the access context isn't known on the client.
- Scan `src/lib/` for existing utilities before creating new types/functions to avoid duplication.
