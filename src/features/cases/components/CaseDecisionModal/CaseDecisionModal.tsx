"use client";

import { useState } from "react";
import { Form } from "react-aria-components";

import { Button } from "@/components/ui/Button/Button";
import { Modal } from "@/components/ui/Modal/Modal";
import { TextField } from "@/components/ui/TextField/TextField";
import { CaseStatusChangePayloadSchema } from "@/features/cases/schemas";
import { createFieldValidator, optionalString } from "@/lib/form-utils";
import { toastError } from "@/lib/toast-utils";

import styles from "./CaseDecisionModal.module.css";

interface CaseDecisionModalProps {
  isOpen: boolean;
  onOpenChange: (isOpen: boolean) => void;
  title: string;
  description: string;
  reasonLabel: string;
  reasonPlaceholder: string;
  confirmLabel: string;
  onConfirm: (reason?: string) => Promise<void>;
}

export function CaseDecisionModal({
  isOpen,
  onOpenChange,
  title,
  description,
  reasonLabel,
  reasonPlaceholder,
  confirmLabel,
  onConfirm,
}: CaseDecisionModalProps) {
  const [reason, setReason] = useState("");
  const [isPending, setIsPending] = useState(false);

  function handleDismiss() {
    if (isPending) return;
    setReason("");
    onOpenChange(false);
  }

  async function handleSubmit(event: React.SyntheticEvent) {
    event.preventDefault();
    if (isPending) return;

    setIsPending(true);
    try {
      await onConfirm(optionalString(reason));
      setReason("");
    } catch {
      toastError(
        "Something went wrong",
        "Please try again. If this keeps happening, refresh the page and try again.",
      );
    } finally {
      setIsPending(false);
    }
  }

  return (
    <Modal title={title} isOpen={isOpen} onOpenChange={handleDismiss} className={styles.modal}>
      <Form onSubmit={handleSubmit}>
        <p className={styles.description}>{description}</p>
        <div className={styles.column}>
          <TextField
            label={reasonLabel}
            value={reason}
            onChange={setReason}
            placeholder={reasonPlaceholder}
            validate={createFieldValidator(CaseStatusChangePayloadSchema.shape.reason)}
            isDisabled={isPending}
            isTextArea
            rows={3}
          />
        </div>
        <div className={styles.actions}>
          <Button variant="secondary" type="button" onPress={handleDismiss} isDisabled={isPending}>
            Cancel
          </Button>
          <Button type="submit" isDisabled={isPending} isPending={isPending}>
            {confirmLabel}
          </Button>
        </div>
      </Form>
    </Modal>
  );
}
