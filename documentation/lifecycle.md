...
Single source of truth in `src/lib/lifecycle.ts` (`CONSULTATION_TRANSITIONS`, `CASE_TRANSITIONS`); feature `status.ts` modules re-export them plus entity copy. Tasks have no transition table — status is derived (`deriveTaskStatus`), never set.
