...
| `requirePermissionOrNull(...)`  | Never            | Same as `requirePermission` but returns `null` on denial instead of throwing. Use in write actions returning `ActionStatusResponse`.                      |
| `assertRecordPermission(...)`   | `"Forbidden"`    | Evaluates a record-scoped permission against an `AccessContext` and throws when denied. Call after loading the context.                                   |
